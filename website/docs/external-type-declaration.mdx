...
The getter prefixes that are tried (`get`, `is` by default) are configurable through
[`SchemaFactory::stripFieldPrefixes()`](other-frameworks.mdx). For instance, adding `has` lets a
`#[SourceField(name: 'stock')]` resolve against a `hasStock()` hasser.
