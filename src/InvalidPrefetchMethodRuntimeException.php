...
class InvalidPrefetchMethodRuntimeException extends GraphQLRuntimeException
{
    ...
    {
        return new self(
            "#[Prefetch] attribute on parameter \${$parameterName} in {$reflector->getDeclaringClass()->getName()}::{$reflector->getName()} specifies a callable that is invalid: {$e->getMessage()}",
            previous: $e,
        );
    }
}
