...
The prefetch callable must be one of the following:
  ...
  - an invokable object: `#[Prefetch(new CommentLoader())]`
  - on PHP 8.5 and later, first-class callable syntax: `#[Prefetch(OtherClass::prefetchMethod(...))]`

First-class callable syntax **cannot** name a non-static method resolved through the container: it
compiles, and then fails when the attribute is read. Keep using the array form for that case — it is
not a workaround for older PHP, it is the only syntax that can express it.

The callable must name a real method, because its own parameters become GraphQL arguments and their
descriptions are read from the method docblock. An inline closure has no such method and is rejected
