...
If the [`@Logged` and `@Right` annotations](authentication-authorization.mdxx#logged-and-right-annotations) are not
...
The `failWith` attribute behaves just like the [`@FailWith` annotation](authentication-authorization.mdxx#not-throwing-errors)
