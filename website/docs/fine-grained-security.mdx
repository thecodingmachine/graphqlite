...
Every form below receives the same `SecurityRuleContext` and must return `bool`. They differ only in
what they can express and which PHP version accepts them.
